...
ZhHant.CONTROLS_FOREACH_TOOLTIP = "遍歷每個陣列中的專案，將變數“%1”設定到該項中，然後執行某些語句。";
...
ZhHant.CONTROLS_FOR_TOOLTIP = "從起始數到結尾數中取出變數“%1”的值，按指定的間隔，執行指定的塊。";
...
ZhHant.LISTS_CREATE_EMPTY_TOOLTIP = "返回一個陣列，長度為 0，不包含任何資料記錄";
...
ZhHant.MATH_RANDOM_FLOAT_TOOLTIP = "返回介於（包含）0.0到1.0之間的亂數。";
...
ZhHant.MIXLY_IR_RECEIVE_RAW = '紅外接收並列印資料（RAW） @腳位';
ZhHant.MIXLY_IR_SEND_RAW = '紅外發射（RAW） @腳位';
...
ZhHant.MIXLY_IR_SEND_RAW_TOOLTIP = '發射RAW紅外資料，4個參數分別是腳位號、陣列、陣列長度、紅外信號頻率';
...
ZhHant.MIXLY_TOOLTIP_BLOCKGROUP_GET_TEM = '返回溫濕度感測器測得的溫度（0~50）';
ZhHant.MIXLY_TOOLTIP_BLOCKGROUP_GET_HUM = '返回溫濕度感測器測得的濕度（20~90%）';
...
ZhHant.MIXLY_TOOLTIP_I2C_MASTER_WRITE = 'I2C主設備發送資料，參數是主設備的位址和要發送的值';
ZhHant.MIXLY_TOOLTIP_I2C_MASTER_READ = 'I2C主設備讀取資料，參數是主設備的位址和要讀取內容的位元組長度';
ZhHant.MIXLY_TOOLTIP_I2C_MASTER_READ2 = '返回從I2C讀取到的資料';
ZhHant.MIXLY_TOOLTIP_I2C_AVAILABLE = '返回是否成功讀取I2C設備資料';
ZhHant.MIXLY_TOOLTIP_I2C_SLAVE_ONRECEIVE = 'I2C從機接收主機產生的時鐘和資料信號，並執行相應操作';
ZhHant.MIXLY_TOOLTIP_I2C_HOWMANY = '返回I2C從機接收位元組數';
ZhHant.MIXLY_TOOLTIP_I2C_SLAVE_ONREQUEST = 'I2C從機根據主機產生的時鐘和資料信號發送，並執行相應操作';
...
ZhHant.MIXLY_TOOLTIP_RFID_INIT = '初始化RFID讀卡器，參數為資料線腳位號、時鐘線腳位號、主輸出從輸出線腳位號和主輸入從輸出先腳位號';
...
ZhHant.MIXLY_TOOLTIP_GPS_INIT = '初始化GPS設備，參數為GPS通信腳位和序列埠傳輸速率';
...
ZhHant.MIXLY_TOOLTIP_DS1302_INIT = '初始化DS1302 RTC時鐘模組，參數為重置線腳位號，資料線腳位號和時鐘線腳位號';
ZhHant.MIXLY_TOOLTIP_DS1307_INIT = '初始化DS1307 RTC時鐘模組，參數為資料線腳位號和時鐘線腳位號';
...
ZhHant.MIXLY_TOOLTIP_LCD_INIT = '初始化LCD顯示幕，設置位址';
ZhHant.MIXLY_TOOLTIP_LCD_INIT2 = '初始化LCD顯示幕，指定時鐘線和資料線連接腳位並設置位址';
ZhHant.MIXLY_TOOLTIP_LCD_INIT3 = '初始化LCD顯示幕，指定所有連接腳位並設置位址';
...
ZhHant.MIXLY_TOOLTIP_4digitdisplay_displayString = 'TM1650四位元數碼管顯示字串（超過四位元則滾動顯示）';
...
ZhHant.MIXLY_TOOLTIP_ETHERNET_INIT = '啟動乙太網服務，參數為乙太網擴展卡的MAC位址，返回1表示成功啟動，0表示失敗';
ZhHant.MIXLY_TOOLTIP_ETHERNET_MACADDRESS = 'MAC地址，格式為XX-XX-XX-XX-XX-XX,X為任意十六進位數';
ZhHant.MIXLY_TOOLTIP_ETHERNET_LOCALIP = '返回本機IP地址';
ZhHant.MIXLY_TOOLTIP_ETHERNET_CONNECT = '連接遠端伺服器，返回1表示成功啟動，其他表示失敗';
...
ZhHant.MIXLY_TOOLTIP_ETHERNET_CLIENT_PRINT = '將資料列印到用戶端連接到的伺服器。發送內容會轉為對應的ASCII碼（例如，數位123作為三個字元“1”、“2”、“3”發送）。';
ZhHant.MIXLY_TOOLTIP_ETHERNET_CLIENT_PRINTLN = '將資料列印到用戶端連接到的伺服器。發送內容會轉為對應的ASCII碼並在最後添加分隔符號（例如，數位123作為三個字元“1”、“2”、“3”發送）。';
...
ZhHant.MIXLY_TOOLTIP_MLX90614_GET_DATA = 'MLX90614紅外測溫傳感器獲取溫度，返回數據的類型為float';
...
ZhHant.MIXLY_4DIGITDISPLAY_TM1637_DISPLAYSTRING_TIP = '少於等於4字元為顯示，超過4個字元為滾動。';
ZhHant.MIXLY_4DIGITDISPLAY_TM1637_DISPLAYSTRING_TIP = '最多顯示4位數字，多餘4位數字無效。';
...
ZhHant.DICTS_CREATE_EMPTY_TOOLTIP = "返回一個空字典，長度為 0，不包含任何資料記錄";
...
ZhHant.INOUT_input_TOOLTIP = '接收鍵盤輸入，以回車結尾，prompt是提供給用戶的輸入提示';
ZhHant.INOUT_raw_input_TOOLTIP = '以字串格式接收鍵盤輸入，以回車結尾，prompt是提供給用戶的輸入提示';
...
ZhHant.MIXLY_TURTEL_GET_SHAPESIZE_TOOLTIP = '獲取海龜當前的圖示大小，返回一個長度為3的元組，分別代表海龜當前垂直方向的拉伸倍數，水準方向上的拉伸倍數以及輪廓寬度值';
...
ZhHant.MIXLY_TURTEL_EVENT_ONKEY_TOOLTIP = '創建一個鍵盤回應事件，字串鍵值可以是普通的按鍵或功能鍵，鍵值為None表示任何按鍵按下都會觸發';
...
ZhHant.MIXLY_TOOLTIP_CHAOSHENGBO = '超聲波測距(cm):trigger接腳位15，echo接腳位14，sclk接腳位13';
...
ZhHant.LISTS_CREATE_PYTHON_EMPTY_TOOLTIP = "返回一個列表，長度為 0，不包含任何資料記錄";
...
ZhHant.MIXLY_TOOLTIP_BLOCKGROUP_GET_TEM_HUM = "返回溫濕度感測器測得的溫度和濕度，溫度範圍：0~50，濕度範圍：20~90%";
...
ZhHant.blockpy_series_create_TOOLTIP = '通過字典或清單創建一個series，索引為預設的自然數';
ZhHant.blockpy_series_create_index_TOOLTIP = '通過字典或清單創建一個series，並指定一個清單作為索引';
...
ZhHant.mixpy_PANDAS_READCSV_TOOLTIP = '將指定的csv檔內容讀入一個變數，變數類型為Dataframe，如選擇包含標題行則默認將csv檔第一列(row)作為標題';
...
ZhHant.mixpy_TURTLE_SAVEFIG_TOOLTIP = "通過海龜螢幕保存繪製的圖形，支援的檔案格式為.eps，需要預先初始化海龜螢幕"
...
ZhHant.blockpy_REQUESTS_GET_TOOLTIP = '利用HTTP協議向指定功能變數名稱發起get請求，將回應結果賦予變數';
...
ZhHant.MIXLY_ESP32_I2C_INIT = "初始化I2C，sda為資料信號，scl為時鐘信號，頻率為資料傳輸速率一般為10k或40k";
ZhHant.MIXLY_ESP32_I2C_SCAN = "搜索I2C匯流排上設備，返回設備位址";
ZhHant.MIXLY_ESP32_I2C_AVAILABLE = "判斷I2C讀取資料是否成功，返回真/假";
ZhHant.MIXLY_ESP32_SPI_INIT_TOOLTIP = "創建SPI對象並設置序列埠傳輸速率(baudrate)，(時鐘)極性(polarity):0/1，(時鐘)相位(phase):0/1，以及時鐘(SCK)，主設備輸出，從設備輸入(MOSI)，主設備輸入，從設備輸出(MISO)腳位";
...
ZhHant.MIXLY_ESP32_LED_SETBRIGHT = "設置內嵌LED燈的當前亮度（0~65535）";
...
ZhHant.MIXLY_ESP32_INOUT_ANALOG_READ_TOOLTIP = "獲取類比輸入裝置的值（0~65535）";
ZhHant.MIXLY_ESP32_INOUT_PWM_ANALOG_WRITE_INIT_TOOLTIP = "初始化某腳位為類比輸出腳位";
ZhHant.MIXLY_ESP32_INOUT_PWM_ANALOG_WRITE_TOOLTIP = "設置某腳位類比輸出的值（0~65535）";
ZhHant.MIXLY_ESP32_INOUT_PWM_ANALOG_WRITE_SET_FREQ_TOOLTIP = "設置某腳位類比輸出的頻率（1～78125）";
...
ZhHant.MIXLY_PYTHON_CONTROLS_RANGE_TOOLTIP = "返回一個整數Range，不包含末尾的數位，模組中的三個參數均應為整數";
ZhHant.MIXLY_PYTHON_CONTROLS_FOR_RANGE_TOOLTIP = "從起始數到結尾數中取出變數“%1”的值，按指定的間隔，執行指定的塊";
...
ZhHant.MIXLY_MIXPY_INOUT_STR_INPUT_TOOLTIP = '接收鍵盤輸入，以回車結尾，內容自動轉換為字串，prompt是提供給用戶的輸入提示';
ZhHant.MIXLY_MIXPY_INOUT_INT_INPUT_TOOLTIP = '接收鍵盤輸入，以回車結尾，內容自動轉換為整數，prompt是提供給用戶的輸入提示';
ZhHant.MIXLY_MIXPY_INOUT_FLOAT_INPUT_TOOLTIP = '接收鍵盤輸入，以回車結尾，內容自動轉換為小數，prompt是提供給用戶的輸入提示';
...
ZhHant.MIXLY_MIXPY_INOUT_PRINT_MANY_TOOLTIP = '列印多個參數，如果參數個數為0，則讓游標下移一列';
...
ZhHant.MPYTHON_BLYNK_SENSOR_DATA_TO_APP_TOOLTIP = '虛擬腳位：V0 ~ V255';
ZhHant.MPYTHON_BLYNK_SETUP_HELPURL = '';
ZhHant.MPYTHON_BLYNK_SETUP_MESSAGE0 = 'Blynk 設置 %1 伺服器 %2 埠 %3 授權碼 %4';
ZhHant.MPYTHON_BLYNK_SETUP_TOOLTIP = '伺服器默認為 blynk-cloud.com，埠默認為 80';
...
ZhHant.MIXLY_AipSpeech_synthesis_TOOLTIP = '將一段文字合成為語音。屬性為一個預設為空的字典，包括spd(語速，取值為0~9，默認為5)、pit(音調，取值為0~9，默認為5)、vol(音量，取值為0~15，默認為5)、per(發音人，0為女聲，1為男聲)';
...
ZhHant.MIXLY_AipImageClassify_advancedGeneral_TOOLTIP = '識別圖片，返回的資料為字典，其中result為結果內容。';
...
ZhHant.MIXLY_AipOcr_TOOLTIP = '識別指定類型圖片中的文字，返回的資料為字典，其中words_result為結果內容。';
ZhHant.MIXLY_AipSpeech_File = '音訊檔';
ZhHant.MIXLY_AipSpeech_asr = '語音辨識';
ZhHant.MIXLY_AipSpeech_ASR_TOOLTIP = '識別語音中的文字。屬性字典中，鍵"dev_pid"對應的值為語言編碼。1536、1537為普通話，1737為英語，1637為粵語，1837為四川話。返回的資料為字典，其中result為結果內容。';
...
ZhHant.MIXLY_AipNlp_TOOLTIP = '返回的資料為字典，其中items為結果內容。';
ZhHant.MIXLY_AipNlp_Sim = '相似度檢測';
ZhHant.MIXLY_AipNlp_Sim_TOOLTIP = '返回的資料為字典，其中score為相似度結果。';
ZhHant.MIXLY_AipNlp_Topic_TOOLTIP = '返回的資料為字典，其中item為結果。lv1_tag_list、lv2_tag_list分別為一級、二級分類結果。';
ZhHant.MIXLY_AipNlp_Summary_TOOLTIP = '返回的資料為字典，其中summary為結果。';
ZhHant.MIXLY_AipNlp_Topic_Title = '標題';
ZhHant.MIXLY_AipNlp_Topic_Content = '正文';
ZhHant.MIXLY_AipFace_match = '人臉比對';
ZhHant.MIXLY_AipFace_match_TOOLTIP = '返回的資料為字典，其中score為相似度結果。';
...
ZhHant.Blynk_connected_Tooltip = "返回Blynk伺服器連接狀態，正常返回 1 反之返回 0";
...
ZhHant.ELIMINATE_HELP = '當你知道在String的開頭或結尾有無關的空白字元並且你想要消除它們時，它很有用。空格是指佔據空間但未被看到的角色。它包括單個空格';
...
ZhHant.DUALL_CORE_MULTITASKING_HELP = 'ESP32雙核多工，同時處理兩個任務互不干擾，優先順序最高為3（若有看門狗被觸發則需要添加僅用於雙核多工的延時函數，延時時間不宜影響其他函數執行）';
...
ZhHant.esp_camera = "ESP32 CAM相機，上傳程式後通過序列埠監視器輸出相機地址流覽器打開即可(默認序列埠傳輸速率115200)";
...
ZhHant.MIXLY_ESP32S2_LED_SETBRIGHT = "設置內嵌LED燈的當前亮度（0~65535）";
...
ZhHant.MIXLY_ESP32_IOT_WIFI_RADIO_CONNECT_TOOLTIP = '連接用戶名為某某，密碼為某某的WiFi';
ZhHant.MIXLY_ESP32_IOT_WIFI_RESET_TOOLTIP = 'WiFi復位';
ZhHant.MIXLY_ESP32_IOT_EMQX_INIT_TOOLTIP = '伺服器地址默認為39.101.161.127，代理網口默認為1883，輸入用戶名，輸入密碼，輸入項目，為WiFi創建連接池，SSL默認上下文';
...
ZhHant.MIXLY_MIXGO_ESPNOW_SEND_TOOLTIP= 'MAC位址為12位，如MAC為"FFFFFFFFFFFF"，則廣播發送';
...
ZhHant.MIXLY_AI_SENSOR_QR_CODE_RECOGNTITION_TOOLTIP = '使用MixGo AI攝像頭識別二維碼，返回包含所有識別結果物件的清單';
...
ZhHant.MIXLY_AI_SENSOR_BAR_CODE_RECOGNTITION_TOOLTIP = '使用MixGo AI攝像頭識別條碼，返回包含所有識別結果物件的清單';
ZhHant.MIXLY_TAG = '標籤碼';
ZhHant.MIXLY_TAG_RECOGNTITION = '標籤碼識別';
ZhHant.MIXLY_AI_SENSOR_TAG_TOOLTIP = '獲取標籤碼物件的相應資訊';
ZhHant.MIXLY_AI_SENSOR_TAG_RECOGNTITION_TOOLTIP = '使用MixGo AI攝像頭識別標籤碼，返回包含所有識別結果物件的清單';
ZhHant.MIXLY_LINE = '直線';
ZhHant.MIXLY_LINE_RECOGNTITION = '直線識別';
ZhHant.MIXLY_AI_SENSOR_LINE_TOOLTIP = '獲取直線物件的相應資訊';
ZhHant.MIXLY_AI_SENSOR_LINE_RECOGNTITION_TOOLTIP = '使用MixGo AI攝像頭識別直線，返回包含所有識別結果物件的清單';
...
ZhHant.MIXLY_AI_SENSOR_CIRCLE_RECOGNTITION_TOOLTIP = '使用MixGo AI攝像頭識別圓形，返回包含所有識別結果物件的清單';
...
ZhHant.MIXLY_AI_SENSOR_RECT_RECOGNTITION_TOOLTIP = '使用MixGo AI攝像頭識別矩形，返回包含所有識別結果物件的清單';
ZhHant.MIXLY_COLOR_RECOGNTITION = '顏色識別';
ZhHant.MIXLY_COLOR_ANALYSIS = '顏色解析';
ZhHant.MIXLY_AI_SENSOR_COLOR_TOOLTIP = '獲取顏色物件的相應資訊';
ZhHant.MIXLY_AI_SENSOR_COLOR_RECOGNTITION_TOOLTIP = '使用MixGo AI攝像頭識別顏色，返回包含所有識別結果物件的清單';
...
ZhHant.MIXLY_AI_SENSOR_MUSIC_RECORD_TOOLTIP = '利用MixGo AI攝像頭錄製音訊，錄製過程程式不阻塞';
ZhHant.MIXLY_YOLO_RECOGNIZE = '外部模型載入';
ZhHant.MIXLY_ANCHOR = '錨點參數';
ZhHant.MIXLY_MODEL_PATH = '模型路徑';
ZhHant.MIXLY_AI_SENSOR_YOLO_RECOGNIZE_TOOLTIP = '利用攝像頭讀取SD卡中的模型檔進行物品識別，返回包含所有識別結果物件的清單';
